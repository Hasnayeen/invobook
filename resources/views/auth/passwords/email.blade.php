@component('layouts.app')
    ...
    <div class="md:bg-white container md:mx-4 md:mx-auto md:shadow-lg rounded md:mt-32 flex md:flex-row flex-col md:max-w-2xl lg:max-w-4xl justify-between">
        ...
        <div class="w-full md:bg-indigo-100 md:w-1/2 p-8">
            <p class="text-sm text-gray-600 pb-2">@lang('auth.Forgot your account’s password or having trouble logging into your account?')</p>
            <p class="text-sm text-gray-600 pb-8">@lang('auth.Enter your email address and we’ll send you a recovery link.')</p>
            ...
            <form role="form" method="POST" action="{{ url('/password/email') }}">
                ...
                <p class="py-4">
                    <button type="submit" class="bg-indigo-600 text-white px-4 py-2 rounded">Send Reset Link</button>
                </p>
            </form>
        </div>
    </div>
