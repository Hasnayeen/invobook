  ...
  <div class="flex flex-col items-center">
    ...
    <div class="p-8 pt-0 min-w-full">
      <div class="text-gray-800 bg-blue-100 px-8 pb-20 leading-normal rounded-lg">
        ...
        <div v-if="user.bio" class="px-8 text-center">
          {{user.bio}}
        </div>
    ...
    <div class="text-gray-800 self-start p-8 bg-gray-100 w-full rounded-b">
      <div>
        ...
        <div class="py-8 -mx-2">
          <a :href="'/projects/' + project.id" class="bg-teal-400 text-white rounded-full font-medium px-3 py-2 mx-2 no-underline" v-for="project in user.projects">
            {{project.name}}
          </a>
          <a :href="'/teams/' + team.id" class="bg-indigo-400 text-white rounded-full font-medium px-3 py-2 mx-2 no-underline" v-for="team in user.teams">
            {{team.name}}
          </a>
        </div>
      </div>
