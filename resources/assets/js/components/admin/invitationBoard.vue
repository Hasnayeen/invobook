...
<div :class="{'hidden': (activeColumn !== 'invitations')}" class="bg-gray-100 rounded-b">
  ...
  <div class="bg-white p-8 pt-4 rounded-b">
    ...
    <div class="flex flex-row items-center py-4">
      <label for="datepicker" class="text-gray-700 text-lg pr-4">Expiry Date <span class="text-sm">(optional)</span>:</label>
      <datepicker v-model="expiryDate" ref="dueOnDate" :disabled-dates="disabledDates" placeholder="Select Date" name="datepicker" format="yyyy-MM-dd" input-class="appearance-none bg-gray-200 text-gray-800" wrapper-class="appearance-none block w-80 bg-gray-200 text-gray-800 border border-gray-200 rounded py-3 px-4"></datepicker>
    </div>
    <div class="flex flex-row items-center pt-4">
      <input type="text" name="" id="shareableLink" :value="shareableLink" placeholder="click generate to create link" class="border-gray-500 border bg-gray-200 text-indigo-700 w-96 px-4 py-3 rounded-l" readonly>
      <div v-if="shareableLink" @click="copyToClipboard" class="px-5 py-3 border-t border-b border-gray-500 cursor-pointer" title="COPY">
