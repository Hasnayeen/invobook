        ...
        <template v-for="activity in value">
          ...
          <div class="flex flex-row items-center">
            ...
            <div class="flex-grow bg-white border px-4 md:px-8 py-4 rounded">
              <a :href="'/users/' + activity.causer.username" class="text-blue-500 font-medium cursor-pointer no-underline">{{ activity.causer.name }}</a>
              {{ activity.description }} {{ activity.subject_type }}
              <a v-if="activity.subject" :href="activity.subject_type + 's/' + activity.subject_id" class="text-blue-500 font-medium cursor-pointer no-underline">{{ activity.subject.name }}</a>
            </div>
          </div>
        </template>
