...
<div class="relative bg-white shadow-md w-64 h-64 flex flex-col justify-center items-center text-center rounded m-4">
  ...
  <div class="border-t w-full h-16 flex flex-row justify-start items-center px-4">
    ...
    <span v-if="project.members.length > 5" class="bg-gray-200 border-teal-500 border p-2 rounded-full">{{ project.members.length - 5 }}+</span>
    <span v-if="project.members.length == 0" class="text-gray-600 text-center">No members yet</span>
  </div>
</div>
</template>
