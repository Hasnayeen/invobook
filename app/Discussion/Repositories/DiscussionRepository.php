...
class DiscussionRepository
{
    ...
    public function getAllDraftDiscussionWithCreator($type, $entityId, $cycleId = null)
    {
        $query = $cycleId ? $this->model->where('cycle_id', $cycleId) : $this->model->whereNull('cycle_id');

        return $query->where(['discussionable_type' => $type, 'discussionable_id' => $entityId, 'draft' => true, 'archived' => false, 'cycle_id' => $cycleId])->with(['creator:id,avatar,name,username', 'category:id,name'])->get(['id', 'name', 'content', 'raw_content', 'posted_by', 'created_at', 'category_id', 'cycle_id']);
    }
